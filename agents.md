...
1. [x] Clicking in rerun/approve build is not triggering the action. This is a critical bug that needs to be fixed as soon as possible. It may be related to the fact that the API calls are not being made correctly or that the state is not being updated properly after the action is triggered.
