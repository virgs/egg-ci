...
1. [x] The sync countdown display doesn't have to be updated every second. Instead, show messages like ("Syncing in less than 30 seconds", "Syncing in less 1 minute", etc. Or something smaller bug as intuitive). This will reduce unnecessary updates and provide a clearer indication of when the next sync will occur without overwhelming the user with constant changes. In the projects page, in the menu each project has, between SELECT APPROVAL JOBS and EXCLUDE PROJECT, add a new divider and a new option "Set sync frequency" or something with the same meaning. When the user clicks on this option, a small modal should open with a form that allows the user to set the sync frequency for that project. The form should have a dropdown with the options (30 seconds, 1 minute, 2 minutes, 5 minutes, 10 minutes). Once the user submits the form, the new sync frequency should be saved to the project configuration and the synchronization mechanism should be updated accordingly to reflect the new frequency. This will allow users to customize how often their projects are synced based on their specific needs and preferences.
