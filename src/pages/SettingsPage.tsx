        ...
        return (
            <ul className="list-group list-group-flush">
                {(projects || []).map((project) => {
                    ...
                    const versionControlComponent = versionControl ? (
                        new VersionControlComponent(versionControl).getIcon()
                    ) : (
                        <></>
                    )
                    const id = `${project.vcsType}/${project.username}/${project.reponame}`
                    return (
                        <li key={id} className="list-group-item d-flex align-items-center" style={{ height: '80px' }}>
                            <input
                                className="form-check-input"
                                type="checkbox"
                                id={id.concat('-checkbox')}
